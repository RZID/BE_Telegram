...
module.exports = {
    login: (req, res) => {
        model.login(req.body.email).then(async (response) => {
            if (response.length < 1) {
                return responser.unauthorized(res, 'User unregistered')
            } else {
                const checkPass = await bcrypt.compare(req.body.pass, response[0].pass_user)
                if (checkPass) {
                    const userData = {
    ...
    register: async (req, res) => {
        ...
        model.register(req.body.name, req.body.email, req.body.pass).then((response) => {
            responser.ok(res, 'Succesfully registered! Check your email for verification')
        }).catch((err) => {
            if (err.message = 409) {
    ...
    getUserData: (req, res) => {
        model.getUserData(req.body.email).then(result => {
    ...
    setProfile: (req, res) => {
        if (req.file) {
            if (req.body.email) {
                model.insertPhoto(req.body.email, req.file.filename).then((resolve) => {
    ...
    setName: (req, res) => {
        if (req.body.email && req.body.name) {
            model.setName(req.body.email, req.body.name).then((response) => {
